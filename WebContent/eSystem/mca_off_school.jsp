...
<form name="xs2" id="xs2" action="mca_off_school.jsp" method=get>
...
<%
    ...
    Student[] st = ezsvc.searchStudent(searchWord, studentIds, orderNum, "studentStatus in (3,4,99)", _ws.getStudentBunitSpace("bunitId"));
    ArrayList<TagMembr> tms = new McaService(0).getCurrentCampusMembrs(_ws.getSessionStudentBunitId());
    // st 包括有設過離校和升級後不在任何 campus 標籤的 (但狀態仍為 3,4)
...
<%
	...
	if(pageNumber !=1)
	{
		...
		out.println("<a href=\"mca_off_school.jsp?pageNum="+lastPage+"&orderNum="+orderNum+"&searchWord="+URLEncoder.encode(searchWord,"UTF-8")+getTagURI(tagIds)+"\">上一頁</a>");
	}
	for(int j=0;j<pageTotal;j++)
	{
		...
		{
			out.println("<a href=\"mca_off_school.jsp?pageNum="+jx+"&orderNum="+orderNum+"&searchWord="+URLEncoder.encode(searchWord,"UTF-8")+getTagURI(tagIds)+"\">"+jx+"</a>");
		}
	}
	if(pageNumber !=pageTotal)
	{
		int lastPage=pageNumber+1;
		out.println("<a href=\"mca_off_school.jsp?pageNum="+lastPage+"&orderNum="+orderNum+"&searchWord="+URLEncoder.encode(searchWord,"UTF-8")+getTagURI(tagIds)+"\">下一頁</a>");
	}
%>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
共計: <font color=blue><%=studentLength%></font> 筆資料 </div>
...
%>
    ...
    {
    %>
    <a href="mca_off_school.jsp?pageNum=<%=pageNumber%>&orderNum=2&searchWord=<%=URLEncoder.encode(searchWord,"UTF-8")+getTagURI(tagIds)%>">性別<img src="images/Upicon2.gif" border=0></a>
    <%
    }else{
    %>
    <a href="mca_off_school.jsp?pageNum=<%=pageNumber%>&orderNum=1&searchWord=<%=URLEncoder.encode(searchWord,"UTF-8")+getTagURI(tagIds)%>">性別<img src="images/Downicon2.gif" border=0></a>
    <%
    }
    %>
    </td>
    ...
    {
    %>
    <a href="mca_off_school.jsp?pageNum=<%=pageNumber%>&orderNum=10&searchWord=<%=URLEncoder.encode(searchWord,"UTF-8")+getTagURI(tagIds)%>">生日<img src="images/Upicon2.gif" border=0></a>
    <%
    }else{
    %>
    <a href="mca_off_school.jsp?pageNum=<%=pageNumber%>&orderNum=9&searchWord=<%=URLEncoder.encode(searchWord,"UTF-8")+getTagURI(tagIds)%>">生日<img src="images/Downicon2.gif" border=0></a>
    <%
    }
    %>
    </td>
...
<tr bgcolor=#ffffff align=left  onmouseover="this.className='highlight'"  onmouseout="this.className='normal2'" valign=middle>
	<td class=es02 nowrap>
        <a href="javascript:openwindow_phm2('modify_mca_student.jsp?studentId=<%=students.get(i).getId()%>','基本資料',700,700,'mcastudentwin');">
            <%=name%> 
            <% 
...
%>	
	...
	<td class=es02 align=middle nowrap>
        ...
        %>
     <!--
        | <a href="bill_detail2.jsp?sid=<%=m.getId()%>&poId=-1&backurl=<%=URLEncoder.encode("mca_off_school.jsp?status=-1"+getTagURI(tagIds))%>">帳單資訊</a>  -->
	    <%
        }
        %>    
        </td>
