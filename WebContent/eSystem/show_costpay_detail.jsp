...
<%		
        ...
        BankAccount b = (BankAccount) BankAccountMgr.getInstance().findX(acctId, _ws.getBunitSpace("bunitId"));
        if (b==null) {
            %><script>alert("資料不存在");history.go(-1)</script><%
            return;
        }
        if (b.getBankAccountName().indexOf("USD$")>=0)
            USD = true;
	}
    else if (bankType==1) {
        Tradeaccount t = (Tradeaccount) TradeaccountMgr.getInstance().findX(acctId, _ws.getBunitSpace("bunitId"));
        if (t==null) {
            %><script>alert("資料不存在");history.go(-1)</script><%
            return;
        }
    }
    else if (bankType==4 || bankType==5) {
        USD = true;
    }
...
<%    
    ...
    item.append("<tr bgcolor=#ffffff align=left  onmouseover=\"this.className='highlight'\"  onmouseout=\"this.className='normal2'\" valign=middle>");
        ...
        item.append("</td>");
        item.append("<td class=es02 align=right valign=top>" + getIncomeNum(cp, USD)+"</td>");
        item.append("<td class=es02 align=right valign=top>" + getCostNum(cp, USD)+ "</td>");
        ...
        item.append("&nbsp;</td>");
        item.append("<td align=right class=es02>");
        item.append("<a target=_blank href=\"print_costpay_receipt.jsp?id=" + cp.getId() + "&r="+new Date().getTime()+"\">" + cpd.getReceiptNo(cp) + "</a>");
        item.append("&nbsp;</td>");
