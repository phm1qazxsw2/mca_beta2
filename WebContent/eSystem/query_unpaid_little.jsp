...
<%
    ...
    ArrayList<BillRecordInfo> brs = BillRecordInfoMgr.getInstance().retrieveListX(
        "billType="+Bill.TYPE_BILLING, "order by month asc", _ws2.getBunitSpace("bill.bunitId"));
    ...
    ArrayList<MembrInfoBillRecord> all_unpaid_bills = MembrInfoBillRecordMgr.getInstance().
        retrieveListX("bill.billType=" + Bill.TYPE_BILLING + " and paidStatus!=" + MembrBillRecord.STATUS_FULLY_PAID +
            ...
            "order by ticketId desc", _ws2.getBunitSpace("bill.bunitId"));
    String backurl = "query_unpaid.jsp";
%>
<br>
<div class=es02>
<b>&nbsp;&nbsp;&nbsp;帳單未繳查詢  區間：<%=sdfx.format(d1)%> 至 <%=sdfx.format(d2)%> </b>
</div>
...
<%
    ...
    ArrayList<Tag> mainTags = ezsvc.getMainTags(_ws2.getStudentBunitSpace("bunitId"));
...
<div class=es02 align=right>合計:<b><%=keys.size()%></b> 筆&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</div>
            ...
            <%
                ...
                for (int i=0; i<bv.size(); i++) {
                    ...
                    if (i==0){
                        ...
                        out.print("<tr bgcolor=#ffffff><td class=es02>");
                    }

                    thisShould+=r.getReceivable();
                    thisPay+=r.getReceived();
                  %>
<%=r.getTicketId()%> <%=r.getBillPrettyName()%> <%=sdf.format(PaymentPrinter.convertToTaiwanDate(r.getBillMonth()))%>
</td>        
...
%>
                <tr align=milddle class=es02>
                    ...
                    <td align=right <%=((thisShould-thisPay)>0)?"bgcolor='#ffffff'":""%>>
                        <b><%=mnf.format(thisShould-thisPay)%></b>
                    </td>
                </tr>
<%                        
                    }
