...
%>
    <li><a href="javascript:setcode('<%=ainfo.getMainSub(main)%>')"><%=ainfo.getMainSub(main)%>-<%=ainfo.getName(main)%></a>        
<%
        if (subs!=null) {
            out.println("<ul>");
...
%>
            <li class="dhtmlgoodies_sheet.gif"><a href="javascript:setcode('<%=ainfo.getMainSub(sub,false)%>')"> <%=ainfo.getMainSub(sub,false)%> <%=ainfo.getMyName(sub)%></a></li>
<%
            }
            out.println("</ul>");
        }
