...
public class DelinquentLetterPrinter
{
    ...
    BillRecord br = null;
       // ticketId
    Map<String, Vector<ChargeItemMembr>> chargeMap = null;
       // chargeKey
    Map<String, Vector<Discount>> discountMap = null;
       // billKey (membrId#billRecordId)
    Map<String, Vector<BillComment>> commentMap = null;
       // chargeKey
    Map<String, Vector<FeeDetail>> feeMap = null;
       // membrId#billId
    Map<String, Vector<MembrInfoBillRecord>> unpaidMap = null;
       // billMap
    ...
    // ######################
    // ## 以下是班級資訊 ##
    // membrId, Vector 里面的是 main type 的 tag
    Map<Integer, Vector<TagMembrStudent>> tagmembrMap = null;
    // ######################
    ...
    public static PaymentPrinter getPdfPrinter(
        ...
        Map<String, Vector<Discount>> discountMap,
           // billKey (membrId#billRecordId)
        Map<String, Vector<BillComment>> commentMap,
           // chargeKey
        Map<String, Vector<FeeDetail>> feeMap,
           // membrId#billId
        Map<String, Vector<MembrInfoBillRecord>> unpaidMap,
...
//System.out.println("## amount1=" + amount1 + " amount2=" + amount2 + " amount3=" + amount3 + " amount4=" + amount4);            
    ...
    public void printPdf7(
        ...
        String dueDate,
        String amountStr,
        Vector<ChargeItemMembr> charges,
        Map<String, Vector<Discount>> discountMap,
        int unpaidAmount,
        String username
        ) throws Exception
    {
       ...
       table = new PdfPTable(widths);
        ...
        parag1=new Paragraph("1. 本收據請勿塗改，塗改無效，如變更金額請與學校助理"+username+"聯絡。 \n"+
                            ...
                            "3.	亦可至其他銀行用匯款(合作金庫銀行中清分行) 或ATM方式轉帳 (銀行代號: 006)。\n"+
                            "4.	若要開立支票，支票抬頭請寫「馬禮遜學校」。",font8NORMAL);
        parag1.setLeading(superscript);
        document.add(parag1);

        Font f_time =new Font(Font.TIMES_ROMAN, 8, Font.NORMAL);
        parag1=new Paragraph("1.	The receipt is not valid if the amount is changed. If you want to make changes, please contact Taichung School assistant ("+username+") for a new receipt. \n"+
...
//        Font underLine= FontFactory.getFont(FontFactory.TIMES_ROMAN, Font.DEFAULTSIZE, Font.UNDERLINE);
        ...
        parag1.add(new Chunk(spaceX+makePrecise("Late Fee:", 20, true, ' '),f_fixed));
        parag1.add(new Chunk(makePrecise(mnf.format(latefee), 15, false, ' ') + "\n",f_fixed));
...
System.out.println("## str_13char=" + str_13char + " amount=" + amount);
    ...
    static void mergeAlias(Vector<ChargeItemMembr> charges, Map<String, Vector<Discount>> discountMap)
        throws Exception
    {
        Vector<ChargeItemMembr> tmp = new Vector<ChargeItemMembr>();
        Map<Integer, Vector<ChargeItemMembr>> chargeMap = new SortingMap(charges).doSort("getAliasId");
        Iterator<Integer> iter = chargeMap.keySet().iterator();
        while (iter.hasNext()) {
            Integer aliasId = iter.next();
            int aid = aliasId.intValue();
            Vector<ChargeItemMembr> vc = chargeMap.get(aliasId);
            if (aid>0) {
                int amount = 0;
                int discount = 0;
                for (int i=0; i<vc.size(); i++) {
                    ChargeItemMembr c = vc.get(i);
                    Vector<Discount> dv = discountMap.get(c.getChargeKey());
                    for (int j=0; dv!=null && j<dv.size(); j++) {
                        Discount d = dv.get(j);
                        discount += d.getAmount();
                    }
                ...
                Vector<Discount> vd = new Vector<Discount>();
                vd.add(d);
        ...
        throws Exception
    {
        ...
        if (charges==null)
            charges = new Vector<ChargeItemMembr>(); // 有可能項目為零可是帳單還在
        //### 看有沒有用 Alias
        Map<Integer, Vector<ChargeItemMembr>> aliasMap = new SortingMap(charges).doSort("getAliasId");
...
//System.out.println("### br.getMonth().getMonth()=" + br.getMonth().getMonth() + " " + sdf.format(br.getMonth()));
        ...
        Vector<MembrInfoBillRecord> unpaid = unpaidMap.get(sinfo.getMembrId());
...
//            && n!=null && n.trim().length()>0 && nn!=null && nn.trim().length()>0));
        ...
        Vector<BillComment> bc = commentMap.get(sinfo.getBillKey());
        if (bc!=null)
        ...
        if(pSystem.getPagetype()==0|| pSystem.getPagetype()==6){ // 6 是通泉草只有里面一點不同
            ...
            printPdf(this.getDocument(),
                ...
                String className = "";
                if (tagmembrMap!=null) {
                    Vector<TagMembrStudent> vt = tagmembrMap.get(new Integer(sinfo.getMembrId()));
                    if (vt!=null)
            ...
            String amountStr = mnf.format(sinfo.getReceivable());
            Vector<ChargeItemMembr> charges = chargeMap.get(sinfo.getTicketId());
            ...
            if (md!=null && md.getType()>0) {
                ...
                mcasvc.breakingBillFees(sinfo, chargeMap, fees, starndardMap, monthlyMap);
                int tuition = fees[0];
                int latefee = fees[1];
                int interest = fees[2];
                ...
                printMCADeferred(this.getDocument(),mcalogo, lastname, firstname,
                    new Date(), grade, sinfo.getBillRecordName(), bu.getLabel(), tuition, latefee, 
                    starndardMap, monthlyMap);
            }
