...
public class McaInterest
{
    ...
    public McaInterest(int tran_id)
        throws Exception
    {
        ...
        throws Exception
    {
        ...
        if (intr_total>0) {
			...
			//else {// total<paidamount, reduce the diff part and make it in student account
			if (paids!=null) {
				BillPaidInfo bp = paids.get(paids.size()-1);
				int diff = paidamount - total;
				if ((bp.getRemain()+diff)>=0) { //2013/9/20 add by peter, 没加就会强迫消掉这张账单 remain变负的
					...
					sbrmgr.executeSQL("update membrbillrecord set paidStatus=" + MembrBillRecord.STATUS_FULLY_PAID + 
						",received=received-" + diff + " where ticketId='" + bill.getTicketId() + "'");
				}
			}
