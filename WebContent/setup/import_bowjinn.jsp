...
<%          
    ...
    try {            
        ...
        Bunit bu = new BunitMgr(tran_id).find("label='天使托兒所'");
        ...
        for (int i=0; i<lines.length; i++) {
            ...
            String className = tokens[30].trim();
            Tag t = tmgr.find("name='" + className + "'");
            if (t==null) {
                t = new Tag();
                t.setName(className);
                t.setBunitId(bu.getId());
                tmgr.create(t);
            }

            String studentName = tokens[0];
            Student2 student = smgr.findX("studentName='" + studentName + "'", bh.getSpace("bunitId", bu.getId()));
            Membr membr = null;
            boolean exist = true;
            if (student!=null) {
                ImportStudent.parseLeader(tokens, student);
                smgr.save(student);
                membr = mmgr.findX("type=" + Membr.TYPE_STUDENT + " and surrogateId=" + student.getId(), 
                    bh.getSpace("bunitId", bu.getId()));
                exist = true;
